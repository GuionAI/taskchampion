...
impl<'a> PgWireTxn<'a> {
    ...
    async fn task_exists(&self, uuid: Uuid) -> Result<bool> {
        ...
        /* task_exists query */
        let row = t.query_one(sql.as_str(), &vals.as_params()).await?;
        /* task_exists get */
    ...
    async fn resolve_project_id(&self, name: &str) -> Result<String> {
        ...
        let row = t.query_opt(sql.as_str(), &vals.as_params()).await?;
        match row {
            Some(row) => {
...
impl StorageTxn for PgWireTxn<'_> {
    async fn get_task(&mut self, uuid: Uuid) -> Result<Option<TaskMap>> {
        ...
        let (sql, vals) = q
            ...
            .build_postgres(PostgresQueryBuilder);
        /* get_task query */
        let rows = t.query(sql.as_str(), &vals.as_params()).await?;
    ...
    async fn create_task(&mut self, uuid: Uuid) -> Result<bool> {
        ...
        let (sql, vals) = Query::insert()
            ...
            .build_postgres(PostgresQueryBuilder);
        /* create_task insert */
        t.execute(sql.as_str(), &vals.as_params()).await?;
        Ok(true)
    }
    ...
    async fn delete_task(&mut self, uuid: Uuid) -> Result<bool> {
        if self.task_exists(uuid).await? {
            ...
            let (sql, vals) = Query::delete()
                ...
                .build_postgres(PostgresQueryBuilder);
            /* delete_task */
            let n = t.execute(sql.as_str(), &vals.as_params()).await?;
            Ok(n > 0)
        } else {
            Ok(false)
        }
